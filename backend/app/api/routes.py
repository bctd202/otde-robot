...
def parlays():
    ...
    candidates = rank_parlays(provider, settings.symbol_list)
    return ParlayResponse(provider_status=provider.status(), candidates=candidates)
...
def dashboard():
    ...
    for q in quotes:
        candles=provider.candles(q.symbol,"1m")
        if not candles:
            continue
        chain=provider.option_chain(q.symbol)
