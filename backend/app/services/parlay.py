...
def rank_parlays(provider: Any, symbols: list[str]) -> list[ParlayCandidateOut]:
    ...
    if provider_status.status == "unavailable":
        ...
        return output
    quotes = {}
    # Prefer one batched market-data request. Fall back to isolated calls when
    # the provider rejects a batch so one malformed flex symbol cannot block it.
    try:
        quotes.update({quote.symbol: quote for quote in provider.quotes(symbols)})
