...
def _freshness(status: ProviderStatus | None) -> str:
    if status is None:
        return "unknown_current"
    suffix = "current" if status.status == "healthy" else status.status
    return f"{status.mode}_{suffix}"


def _mode_reasons(status: ProviderStatus | None) -> list[str]:
    if status is None:
        return ["Provider data mode is unknown"]
    if status.mode == "demo":
        return ["Demo option data is not tradable live data"]
    if status.mode == "mock":
        return ["Non-live provider data is not tradable"]
    if status.mode == "delayed":
        return ["Delayed data is not eligible for actionable trading"]
    if status.mode == "unknown":
        return ["Provider live-versus-delayed entitlement is unknown"]
    return []


def structured_setups(symbol, candles, quote, chain, provider_status: ProviderStatus | None = None):
...
def lottery_candidates(symbol, candles, quote, chain, provider_status: ProviderStatus | None = None):
    ...
    for c in chain:
        ...
        out.append(LotteryOut(symbol=symbol,option_symbol=c.option_symbol,normalized_symbol=c.normalized_symbol,provider=c.provider,data_mode=c.data_mode,verification_status=c.verification_status,verification_reason=c.verification_reason,bid_timestamp=c.bid_timestamp,ask_timestamp=c.ask_timestamp,quote_timestamp=c.timestamp,actionable=c.actionable,right=c.right,strike=c.strike,expiration=c.expiration,bid=c.bid,ask=c.ask,midpoint=mid,last=c.last,total_debit=debit,otm_percent=round(otm,2),delta=c.delta,gamma=c.gamma,theta=c.theta,iv=c.iv,volume=c.volume,open_interest=c.open_interest,spread_percent=sp,underlying_trigger=round(max(lv["session_high"], c.strike-0.8),2),underlying_invalidation=round(lv["vwap"]-0.2,2),break_even=round(c.strike+c.ask,2),estimated_2x_underlying=round(c.strike+c.ask*2,2),estimated_5x_underlying=round(c.strike+c.ask*5,2),estimated_10x_underlying=round(c.strike+c.ask*10,2),explanation="Modeled estimates use simple intrinsic-value thresholds and are not guarantees; 0DTE options can expire worthless.",catalyst="Confirmed momentum runner: above VWAP and opening-range high.",setup_score=round(score,1),max_allocation=20,time_remaining_minutes=355,worthless_reasons=["Underlying may fail to reach strike quickly", "Bid-ask spread and theta decay can erase premium", *_mode_reasons(provider_status)]))
