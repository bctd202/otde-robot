...
def lottery_candidates(symbol, candles, quote, chain):
    ...
    out=[]
    for c in chain:
        if c.bid is None or c.ask is None or c.volume is None:
            continue
        sp=spread_pct(c.bid,c.ask); debit=c.ask*100
        trigger_ok = bullish and c.right=="call"
        reachable = abs(c.strike-quote.price) <= max(quote.price*0.012, 1.5)
        if not trigger_ok or not (c.expiration == quote.timestamp.date() and s.lottery_min_ask <= c.ask <= s.lottery_max_ask and 20 <= debit <= s.lottery_max_debit and c.volume >= s.lottery_min_volume and sp <= s.lottery_max_spread_pct and c.bid > 0 and reachable):
            continue
        ...
        score=min(100, 40 + c.volume/50 + (c.open_interest or 0)/200 + (c.gamma or 0)*300 - sp/2)
