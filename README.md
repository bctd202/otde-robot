...
One process-wide read-through cache shares quotes, 1M candles, expirations, and exact option chains across the scanner, both engines, entry checks, and position management. 5M, 15M, 1H, and 4H views are aggregated locally from the cached 1M session feed. Expirations are cached for the trading day and option chains are requested only after the underlying setup qualifies. The Tradier adapter enforces a configurable 100-request/minute safety budget below the upstream 120-request limit and reports remaining capacity on the board. Browser polling reads persisted scans, lottery histories, performance pages, and server-owned paper-position marks; it does not initiate normal market-data refreshes.
...
Lottery Lab contracts are forward-tracked from their first qualifying scan through the regular-session close, even after they stop qualifying or disappear from the current shortlist. Each saved point includes the exact contract bid, ask, timestamps, spread, Greeks, and underlying price. The chart uses the first qualifying ask as theoretical entry cost and every subsequent bid as the defensible sellable value; it does not claim midpoint or last-trade fills. `/api/lottery-trackers` exposes the daily history and `/api/lottery-trackers/{id}` exposes the line-chart points, peak multiple, and actual 2x/5x/10x timestamps. Tracking begins with migration `0010_lottery_tracker`; earlier ephemeral Lottery Lab cards cannot be reconstructed.

Automated option shadow tracking begins with migration `0011_automated_option_marks` and intentionally does not backfill old signals. It appends an immutable entry mark at the verified ask for the first account-eligible automated BUY per ticker/day, then appends an exit mark using only the exact same contract's first verified Tradier bid after the underlying TARGET, STOP, or 15:45 ET TIMED_EXIT. The collector retries only inside a configurable three-minute window and preserves the last 20 requests/minute for core scanning. A missing, stale, wrong-contract, non-live, or over-lag quote becomes an explicit `OPTION_QUOTE_GAP`; it is never replaced with midpoint, last trade, underlying movement, or a guessed option value. An explicit live zero bid is retained as zero liquidation value. Shadow option dollars and returns remain separate from headline underlying R while coverage is validated; ask-to-bid pricing includes the observed spread, but fees and additional slippage are not yet modeled.
...
Parlay separates **automated live forward tracking**, manually entered paper positions, and historical backtesting. Every qualified live BUY is frozen in a persistent signal ledger even when the user does not enter a paper position. Manual positions remain available in their own operational view but are excluded from automated strategy analytics. Historical runs replay regular-session 1-minute candles chronologically through the same production setup evaluator used by the live scanner and never route a paper or brokerage order.

Historical outcomes are measured on the underlying against the original entry-to-stop distance. **Strategy R** is the primary risk-normalized result (`direction-adjusted price movement / initial underlying risk`). The dashboard also shows **Directional Move** (entry-to-exit underlying percentage, reversed for puts) and **Raw Underlying** (entry-to-exit price percentage, unchanged for calls and puts). The two movement views are not option P/L or portfolio returns; their cumulative values sum per-play percentages. For automated underlying rows, the existing API `result_return_pct` and `cumulative_return_pct` fields are direction-adjusted legacy fields, not raw underlying movement. The original entry and stop are frozen when the signal triggers, so a later management decision cannot rewrite its R result. Tradier plans may return limited or incomplete intraday history and do not provide enough expired option quotes to reconstruct honest contract P&L, so Parlay does not synthesize or claim historical option returns. Partial ranges and per-ticker failures remain visible.

Performance defaults to the 1-Min/true-0DTE strategy and keeps Structured Intraday 5–14 DTE results in a separate view. The account-like headline keeps the first actual BUY per ticker and trading day; repeated engine fires remain in the raw ledger and are reported as a comparison, not counted as independent positions. Zero-risk plans, cross-session historical exits, and scanner data gaps remain auditable but are excluded from result calculations without rewriting historical rows.

Metrics exclude open and quality-excluded signals from win rate, average/cumulative R, profit factor, and maximum drawdown. Win rate is positive-R completed signals divided by eligible completed signals; profit factor is gross positive R divided by absolute gross negative R; drawdown is the largest peak-to-trough decline in the chronological cumulative-R curve. MFE and MAE are the greatest favorable and adverse underlying excursions divided by initial risk. Duration is trigger-to-exit minutes.
...
Open **Backtest Lab**, select dates and configured tickers, and choose **Run Backtest**. The latest completed result stays visible, overlapping submissions are rejected, and saved history includes requested/available ranges plus ticker failures. Open **Play History & Performance** for daily and cumulative graphs plus paginated, first-position-per-ticker/day cards, with true 0DTE and Structured Intraday kept separate. Strategy R is the default; market movement and the forward-only option shadow are labeled separately. Expand a card for its quality eligibility, frozen strategy version, original risk, conditions, option-at-trigger snapshot, and conservative-resolution marker.
