...
Open **Backtest Lab**, select dates and configured tickers, and choose **Run Backtest**. The latest completed result stays visible, overlapping submissions are rejected, and saved history includes requested/available ranges plus ticker failures. Open **Performance** for automated, first-position-per-ticker/day evidence, with true 0DTE and Structured Intraday kept separate. R Multiple is the default; Underlying % is available but is not option-contract P/L. Expand a row for its quality eligibility, frozen strategy version, original risk, conditions, option-at-trigger snapshot, and conservative-resolution marker.
