...
Local-first 0DTE lottery-options signal research dashboard. Parlay is **paper-only**: there is no brokerage client, account access, order router, or live-order execution path.
...
## Tradier market data and Parlay ranking

Mock mode remains the default. To enable Tradier market data, set environment variables without committing the token:
...
SYMBOLS=SPY,QQQ,IWM,TSLA,NVDA,AAPL,AMZN,META,MSFT,GOOGL,AVGO,IBIT
```

For Tradier sandbox data, use `TRADIER_BASE_URL=https://sandbox.tradier.com/v1`. Parlay uses market-data endpoints only; it has no account or order endpoints. It discovers today's expiration in America/New_York before requesting each chain and never substitutes mock data after a live-provider failure.

`GET /api/parlays` returns every configured symbol ranked best to worst, including explicit unavailable rows. Scores are deterministic research context—not probabilities, recommendations, or live orders. Categories are `PLAY` (85–100), `WATCH CLOSELY` (70–84), `DEVELOPING` (55–69), and `PASS` (below 55).
