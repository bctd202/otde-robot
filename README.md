...
Data labels are explicit: **verified live** is current Tradier data, **verified delayed** is Tradier data with its delay shown, **demo** is intentionally generated mock data, **legacy/unknown** covers older records without auditable provenance, and **unavailable/unverified** cannot be acted on. Synthetic option cards require both `MARKET_DATA_PROVIDER=mock` and `ENABLE_DEMO_OPTION_CONTRACTS=true`. Mock underlying mode alone returns no option chain or trade-looking option fields. Explicit option demo mode displays `DEMO MODE — MOCK OPTION DATA — DO NOT TRADE`, never describes itself as live or delayed, and disables paper entry. The provider factory does not fall back to mock when Tradier or another provider fails.
...
The application setting and production image previously defaulted to the deterministic mock provider. That provider constructed synthetic option symbols, strikes, quotes, Greeks, volume, and open interest whenever mock underlying mode was selected, while Lottery Lab rendered those fields without an expiration/OCC/provenance trust treatment. Verification also copied the same symbol into a second field rather than parsing OCC components, and Tradier bid/ask freshness was incorrectly represented by the last-trade timestamp. These were configuration and validation defects, not a Tradier-to-mock fallback. Production now defaults unavailable, mock option generation has a separate explicit gate, compact OCC fields are independently parsed, and bid/ask timestamps are validated separately.
