...
- Automated performance is measured on the underlying in R because automated option-exit quotes are not recorded. It is not option-contract P/L. Headline analytics use the first BUY per ticker/day, exclude manual positions, and quarantine cross-session, data-gap, and zero-risk outcomes.
