...
- Headline automated performance remains underlying-path R, not option-contract P/L. Forward-only automated option evidence records the verified entry ask and first verified exit bid, but cannot backfill older plays and does not model fees, additional slippage, queue position, or partial fills. It remains a separately labeled shadow series. Headline analytics use the first BUY per ticker/day, exclude manual positions, and quarantine cross-session, data-gap, and zero-risk outcomes.
