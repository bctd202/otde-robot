...
export const getPerformance=(strategyMode:'ONE_MIN_0DTE'|'STRUCTURED_INTRADAY'='ONE_MIN_0DTE',options:{page?:number;pageSize?:number;view?:'ALL'|'OPEN'|'COMPLETED'|'EXCLUDED'}={})=>{
  const params=new URLSearchParams({strategy_mode:strategyMode,page:String(options.page??1),page_size:String(options.pageSize??25),view:options.view??'ALL'});
