...
export function ParlayBoard({data,updated,refreshing,stale,onRetry,positions=[],positionsStale=false,onPaperEnter,onPaperExit,enteringSymbol}:ParlayBoardProps) {
  const groups=useMemo(()=>({
    ...
    waiting:data.candidates.filter(item=>item.signal_status==='WATCH'&&lifecycleIsCurrent(item)).sort((a,b)=>a.ranking_position-b.ranking_position),
    noTrade:data.candidates.filter(item=>!isVerifiedActionable(item)&&!(item.signal_status==='WATCH'&&lifecycleIsCurrent(item))),
  }),[data.candidates]);
  ...
  return <section id="parlay" className="parlay-board" aria-labelledby="parlay-title">
    ...
    <div className="board-meta"><p className="paper-notice"><strong>Paper only</strong><span>No live orders are placed.</span></p><p className="freshness" aria-label="Aggregate scanner health">Scanner health: <strong>{scannerHealth.candidate_count} candidates</strong><span>{scannerHealth.unavailable_candidate_count} unavailable</span><span>Provider {scannerHealth.provider_status}</span><span>Engine {scannerHealth.engine_status??'unknown'}</span>{scannerHealth.evaluation_candle_at&&<span>Completed candle {new Date(scannerHealth.evaluation_candle_at).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:'America/New_York'})} ET</span>}</p></div>
