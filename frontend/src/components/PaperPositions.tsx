  ...
  return <div className="paper-positions-region">
    <section className="paper-positions" aria-labelledby="open-paper-title">
      ...
      {active.length===0?<p className="paper-empty">No open paper positions.</p>:<div className="position-grid">{active.map(position=><article className={`position-card decision-${position.decision_status.toLowerCase()}`} key={position.id}>
        <header><div><h3>{position.symbol} <span>{position.direction.toUpperCase()}</span></h3><small>{position.strategy_mode==='STRUCTURED_INTRADAY'?'Structured Intraday':'1-Min / 0DTE'} · {position.strategy_version}</small><strong>{position.option_symbol}</strong></div><span className="decision-badge">{position.decision_status.replace('_',' ')}</span></header>
    ...
    {expired.length>0&&<details className="recent-exits position-history expired-positions"><summary><span><small>HISTORY</small><h2 id="expired-positions-title">Expired Paper Positions</h2></span><b>{expired.length} positions</b></summary><div aria-labelledby="expired-positions-title">{expired.map(position=><article key={position.id}><strong>{position.symbol} {position.direction.toUpperCase()}</strong><span>Expired {formatDateOnly(position.expiration)}</span><span>Last-known option {price(position.current_option_price)}</span><span>Last-known underlying {price(position.current_underlying_price)}</span><span>Historical / stale — no settlement price recorded</span><time>Last marked {formatEasternDateTime(position.last_marked_at)}</time></article>)}</div></details>}
    {closed.length>0&&<details className="recent-exits position-history"><summary><span><small>HISTORY</small><h2 id="recent-exits-title">Recent Paper Exits</h2></span><b>{closed.length} positions</b></summary><div aria-labelledby="recent-exits-title">{closed.map(position=><article key={position.id}><strong>{position.symbol} {position.direction.toUpperCase()}</strong><span>Entry {price(position.entry_option_price)}</span><span>Exit {price(position.exit_option_price)}</span><span>Realized {position.realized_pnl===null?'—':dollars.format(position.realized_pnl)}</span><span>{position.exit_reason}</span><time>{formatEasternDateTime(position.opened_at)} → {formatEasternDateTime(position.closed_at)}</time></article>)}</div></details>}
  </div>;
}
