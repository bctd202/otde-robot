...
export function Performance(){
  ...
  return <section id="performance" className="performance-page"><p className="eyebrow">Normalized strategy evaluation</p><h2>Performance</h2><p>Compare both engines in the same journal. Outcomes can be viewed against the original stop distance or as the underlying return.</p>
    ...
    {data&&<><div className="performance-metrics">{Object.entries(selectedMetrics).map(([key,value])=><div className="metric" key={key}><span>{labels[key]}</span><strong>{value??'N/A'}</strong></div>)}</div><div className="ledger" role="table"><div className="ledger-row ledger-head"><b>Ticker</b><b>Strategy / setup</b><b>Trigger (ET)</b><b>Plan</b><b>Outcome</b></div>{rows.map(row=>{const outcome=unit==='R'?row.result_r:row.result_return_pct;return <div key={row.signal_id}><button className="ledger-row" onClick={()=>setExpanded(expanded===row.signal_id?'':row.signal_id)}><b>{row.ticker}</b><span>{strategyName(row.strategy_mode)} · {row.direction} · {row.setup_type}</span><span>{new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',dateStyle:'short',timeStyle:'short'}).format(new Date(row.triggered_at))}</span><span>{row.entry_price.toFixed(2)} / {row.stop_price.toFixed(2)} / {row.target_price.toFixed(2)}</span><span>{row.exit_reason} · {outcome==null?'—':`${outcome.toFixed(2)}${suffix}`}{row.user_entered?' · PAPER':''}</span></button>{expanded===row.signal_id&&<pre className="audit-view">{JSON.stringify({strategy_mode:row.strategy_mode,strategy_version:row.strategy_version,initial_risk_points:row.initial_risk_points,initial_risk_pct:row.initial_risk_pct,strategy:row.strategy_snapshot,conditions:row.condition_snapshot,option_at_trigger:row.option_snapshot,conservative_same_candle:row.conservative_same_candle},null,2)}</pre>}</div>})}</div></>}
  </section>;
}
