...
export function App() {
  ...
  return <main>
    ...
    <div className={`mock-banner ${isMock?'mock':isDelayed?'delayed':'live'}`}><strong>{isMock&&dashboard.provider_status.message.includes('DEMO MODE')?'DEMO MODE — MOCK OPTION DATA — DO NOT TRADE':isMock?'MOCK DATA — NOT LIVE':isDelayed?'DELAYED MARKET DATA':'LIVE MARKET DATA'}</strong><span>{dashboard.provider_status.message}</span></div>
