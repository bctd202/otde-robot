...
export function App() {
  ...
  return <main>
    ...
    <div className="grid"><Card id="charts" title="Chart Workspace"><CandlestickChart symbol="SPY" currentPrice={dashboard.quotes.find(quote=>quote.symbol==='SPY')?.price ?? dashboard.levels.SPY.vwap} levels={dashboard.levels.SPY} directionalBias={dashboard.directional_bias.SPY}/></Card><Card id="journal" title={`Signal Journal · ${journal.length}`}>{journal.length===0?<p>Run the seed command to create sample signals.</p>:journal.slice(0,6).map(row=><div className="journal-row" key={row.id}><span>{row.symbol}</span><strong>{row.signal_type.replace('_',' ')}</strong><small>{row.status}</small></div>)}</Card><Card id="analytics" title="Seeded Paper Analytics">{analytics&&<><div className="analytics-grid"><Metric label="Samples" value={analytics.sample_size}/><Metric label="Win rate" value={`${analytics.win_rate}%`}/><Metric label="Profit factor" value={analytics.profit_factor ?? 'N/A'}/><Metric label="Expectancy" value={`${analytics.expectancy}%`}/></div><p>{analytics.message}</p><small>Minimum sample size: {analytics.minimum_sample_size}; promising: {analytics.statistically_promising?'yes':'no'}.</small></>}</Card></div>
