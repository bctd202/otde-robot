...
export function App() {
  ...
  return <main>
    ...
    <div className={`mock-banner ${isMock?'mock':isDelayed?'delayed':'live'}`}><strong>{isMock?(dashboard.provider_status.demo_options_enabled?'DEMO MODE — MOCK OPTION DATA — DO NOT TRADE':'MOCK UNDERLYING DATA — OPTION CONTRACTS DISABLED'):isDelayed?'DELAYED MARKET DATA':'LIVE MARKET DATA'}</strong><span>{dashboard.provider_status.message}</span></div>
    ...
    <Card id="lottery" title={`Lottery Lab · ${dashboard.lottery_setups.length}`} danger><div className="risk-warning"><strong>Highly speculative. Most lottery contracts are expected to expire worthless.</strong><span>Maximum modeled loss is the full debit shown. Estimates are not guarantees.</span></div>{dashboard.lottery_setups.length===0?<p><strong>No verified contract available.</strong> Underlying market context remains visible above; no option quote is being inferred.</p>:dashboard.lottery_setups.map(l=><article className="lotto" key={`${l.symbol}-${l.strike}-${l.right}`}><div><span className="tag orange">{l.symbol} · {formatDateOnly(l.expiration)} · ${l.strike} {l.right.toUpperCase()}</span><h3>Momentum runner · score {l.setup_score}</h3><strong className={`verification ${l.verification_status}`}>{l.actionable?'TRADIER VERIFIED':l.data_mode==='mock'?'DEMO DATA':'UNVERIFIED'}</strong><p>OCC: {l.option_symbol}</p><p>{l.provider} · {l.data_mode} · Quote as of {formatEasternTime(l.quote_timestamp)}</p>{!l.actionable&&<p><strong>No verified contract available</strong> — {l.verification_reason}</p>}<p>{l.explanation}</p></div><div className="trade-grid"><Metric label="Bid / ask" value={`${l.bid} / ${l.ask}`}/><Metric label="Maximum loss" value={`$${l.total_debit}`}/><Metric label="Spread" value={`${l.spread_percent}%`}/><Metric label="Delta / gamma" value={`${l.delta} / ${l.gamma}`}/><Metric label="Trigger / invalid" value={`${l.underlying_trigger} / ${l.underlying_invalidation}`}/><Metric label="2x / 5x / 10x est." value={`${l.estimated_2x_underlying} / ${l.estimated_5x_underlying} / ${l.estimated_10x_underlying}`}/></div><ul>{l.worthless_reasons.map(reason=><li key={reason}>{reason}</li>)}</ul></article>)}</Card>
