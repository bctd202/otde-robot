...
export interface PerformanceMetrics { total_triggered_signals:number;resolved_signals:number;open_signals:number;targets_hit:number;stops_hit:number;timed_exits:number;invalidated_missed:number;data_gap_signals:number;quality_exclusions:number;wins:number;losses:number;breakeven:number;win_rate:number;average_r:number;cumulative_r:number;profit_factor:number|null;average_win_r:number;average_loss_r:number;maximum_drawdown_r:number;exposure_ticker_days:number;exposure_minutes:number;average_duration:number;average_mfe:number;average_mae:number;average_return_pct?:number;cumulative_return_pct?:number;maximum_drawdown_pct?:number }
export interface AutomatedOptionShadow {
  status:'ACTIVE'|'EXIT_PENDING'|'CLOSED'|'OPTION_QUOTE_GAP';option_symbol:string;entry_ask:number;entry_bid:number|null;
  entry_quote_at:string|null;entry_cost_dollars:number;exit_reason:string|null;exit_bid:number|null;exit_ask:number|null;
  exit_quote_at:string|null;quote_lag_seconds:number|null;exit_value_dollars:number|null;pnl_dollars:number|null;
  return_percent:number|null;gap_reason:string|null;entry_basis:'ASK';exit_basis:'BID';contract_multiplier:number;
  fees_modeled:false;additional_slippage_modeled:false;paper_only:true;
}
export interface AutomatedOptionShadowMetrics {
  tracked_positions:number;closed_with_quote:number;quote_gaps:number;active_positions:number;exit_pending:number;
  untracked_selected_positions:number;quote_coverage_percent:number;wins:number;losses:number;cumulative_pnl_dollars:number;
...
export interface PerformanceSignal { signal_id:string;source:'LIVE'|'BACKTEST'|'UNKNOWN'|'LEGACY';ticker:string;direction:'CALL'|'PUT';setup_type:string;strategy_mode:StrategyMode;strategy_version:string;trading_date:string;triggered_at:string;entry_price:number;stop_price:number;target_price:number;exit_at:string|null;exit_price:number|null;exit_reason:string;result_r:number|null;result_return_pct:number|null;return_basis?:'UNDERLYING'|'PAPER_OPTION';paper_entry_option_price?:number|null;paper_exit_option_price?:number|null;initial_risk_points:number;initial_risk_pct:number;mfe_return_pct:number;mae_return_pct:number;duration_minutes:number|null;score:number;mfe_r:number;mae_r:number;user_entered:boolean;analytics_eligible:boolean;analytics_exclusion_reason:string|null;option_snapshot:Record<string,unknown>|null;strategy_snapshot:Record<string,unknown>;condition_snapshot:Record<string,unknown>;conservative_same_candle:boolean;automated_option_shadow:AutomatedOptionShadow|null }
export interface PerformanceDailyPoint {trading_date:string;result_r:number;return_pct:number;cumulative_r:number;cumulative_return_pct:number;resolved:number;wins:number;losses:number;option_pnl_dollars:number;cumulative_option_pnl_dollars:number;option_closed:number}
export interface MarketMovementDailyPoint {trading_date:string;raw_move_pct:number;directional_move_pct:number;cumulative_raw_move_pct:number;cumulative_directional_move_pct:number;resolved:number}
export interface MarketMovementData {resolved_with_prices:number;average_directional_move_pct:number;positive_direction_rate:number;cumulative_directional_move_pct:number;net_raw_underlying_move_pct:number;average_raw_underlying_move_pct:number;daily:MarketMovementDailyPoint[]}
